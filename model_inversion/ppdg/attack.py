...
from PPDG_MI.high_resolution.attacks.final_selection import perform_final_selection
from PPDG_MI.high_resolution.attacks.optimize import Optimization
from PPDG_MI.high_resolution.datasets.custom_subset import ClassSubset
from PPDG_MI.high_resolution.metrics.classification_acc import ClassificationAccuracy
from PPDG_MI.high_resolution.metrics.fid_score import FID_Score
from PPDG_MI.high_resolution.metrics.prcd import PRCD
from PPDG_MI.high_resolution.utils.attack_config_parser import AttackConfigParser
from PPDG_MI.high_resolution.utils.datasets import (create_target_dataset, get_facescrub_idx_to_class,
                            get_stanford_dogs_idx_to_class)
from PPDG_MI.high_resolution.utils.stylegan import create_image, load_discrimator, load_generator
from PPDG_MI.high_resolution.utils.wandb import *
from PPDG_MI.high_resolution.utils import w_projector_lpips as w_projector

from PPDG_MI.high_resolution.losses import l2_loss
from PPDG_MI.high_resolution.losses import mmd_loss
from PPDG_MI.high_resolution.losses import ct_loss
from PPDG_MI.high_resolution.losses.localitly_regulizer import Space_Regulizer
import PPDG_MI.high_resolution.dnnlib as dnnlib

from PPDG_MI.high_resolution.utils.models_utils import toogle_grad, set_requires_grad, write_list
...
def log_nearest_neighbors_local(imgs, targets, eval_model, img_size, seed, save_dir,
                                nrow, round_num, **kwargs):
    target_id = kwargs['target_id']
    target_dataset = kwargs['target_dataset']

    # Find closest training samples to final results
    evaluater = DistanceEvaluation(eval_model, None, img_size, None, target_dataset,
                                   seed)
...
def log_final_images_local(imgs, predictions, max_confidences, target_confidences,
                           idx2cls, save_dir, nrow, round_num, **kwargs):
    target_id = kwargs['target_id']
...
def calc_loss(generated_images, target_images, target_features, method, G, w_batch, **kwargs):
    config = kwargs['config']
    lpips_loss = kwargs['lpips_loss']
    space_regulizer = kwargs['space_regulizer']
...
def attack_single_id(target_model, w, synthesis, discriminator, targets_single_id, **kwargs):
    config = kwargs['config']
    current_time = kwargs['current_time']
    target_id = kwargs['target_id']
    batch_size = kwargs['batch_size']
    args = kwargs['args']
    num_ws = kwargs['num_ws']
    round_num = kwargs['round_num']
    num_epochs = kwargs['num_epochs']
    # target_dataset_name = kwargs['target_dataset_name']
    gpu_devices = kwargs['gpu_devices']
    batch_size_single = kwargs['batch_size_single']
    idx_to_class = kwargs['idx_to_class']
    evaluation_model = kwargs['evaluation_model']
    img_size = kwargs['img_size']
    target_dataset = kwargs['target_dataset']
    wandb_run_id = kwargs['wandb_run_id']

    if config.logging:
        save_dir = os.path.join('results', wandb_run_id, f'{target_id: 03d}')
        Path(f"model_inversion/ppdg/results/{wandb_run_id}/{target_id:03d}").mkdir(parents=True, exist_ok=True)
    ...
    final_w_path = f" "
    # final_w_path = f"model_inversion/ppdg/results/final_w/CelebA_ResNet18_id_0_99_35iters.pt"
    # final_w_path = f"model_inversion/ppdg/results/final_w/CelebA_resnest50_id_0_99_35iters.pt"
    if round_num == 0 and os.path.exists(final_w_path):
        ...
        # final_w_path = f"model_inversion/ppdg/results/20240430_120537_id_0_99_35iters_baseline/
    ...
    if config.logging:
        optimized_w_path_selected = f"model_inversion/ppdg/results/{wandb_run_id}/{target_id:03d}/final_w_selected_{round_num + 1}.pt"
    ...
    ####################################
    try:
        # evaluation_model = config.create_evaluation_model() #! we created it alreaedy
        ...
        #! this part isn't needed since we always do final selection
        ...
        if config.final_selection:
            ...
            # add to a CSV that saves acc_top1, acc_top5, avg_total_conf, and avg_correct_conf for each class, to be averaged later
            acc_csv_path = f"model_inversion/ppdg/results/{wandb_run_id}/accuracy_summary_r{round_num + 1}.csv"
            file_exists = os.path.isfile(acc_csv_path)
            with open(acc_csv_path, mode='a', newline='') as acc_file:
                acc_writer = csv.writer(acc_file)
                if not file_exists:
                    acc_writer.writerow(['target_id', 'acc_top1', 'acc_top5', 'avg_correct_conf', 'avg_total_conf'])
                acc_writer.writerow([target_id, acc_top1, acc_top5, avg_correct_conf, avg_total_conf])

            # log precision list
            if config.logging:
                _ = write_list(
                    f'model_inversion/ppdg/results/{wandb_run_id}/precision_list_r{round_num + 1}',
                    precision_list
                )
    ...
    try: #! assumes we have a facial dataset
        ...
        evaluater_facenet = DistanceEvaluation(facenet, synthesis, img_size,
        ...
        if config.logging:
            _ = write_list(
                f'model_inversion/ppdg/results/{wandb_run_id}/distance_facenet_r{round_num + 1}',
                mean_distances_list)
    ...
    if config.logging:
        ...
        log_final_images_local(log_imgs,
                               log_predictions,
                               log_max_confidences,
                               log_target_confidences,
                               idx_to_class,
                               f"model_inversion/ppdg/results/{wandb_run_id}/{target_id:03d}",
                               num_imgs,
                               round_num,
                               **kwargs)
...
def attack(config, target_dataset, target_model, evaluation_model, target_config, wandb_run=None):
    ...
    os.makedirs("model_inversion/ppdg/results", exist_ok=True)

    args_dict = config._config['args'] #! for compatibility with our Hydra setup instead of using their parser
    ...
    # target_model = config.create_target_model() #! target_model passed as an argument 
    target_model_name = target_model.name
    # target_dataset_name = config.get_target_dataset() #! passed as an argument
    ...
    img_size = 160 #! why 160?

    # # Build the datasets
    # if target_dataset_name == 'facescrub':
    #     transform = T.Compose([
    #         T.Resize((img_size, img_size), antialias=True),
    #         T.ToTensor(),
    #         T.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
    #     ])
    # elif target_dataset_name == 'celeba_identities':
    #     transform = T.Compose([
    ...
    #         T.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
    #     ])
    # elif 'stanford_dogs' in target_dataset_name:
    #     transform = T.Compose([
    #         T.Resize((img_size, img_size), antialias=True),
    #         T.ToTensor(),
    #         T.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
    ...
    # else:
    #     raise RuntimeError(
    #         f'{target_dataset_name} is not a valid dataset name. Chose of of [facescrub, celeba_identities, stanford_dogs].'
    #     )
    # target_dataset, _, _ = get_datasets(target_config, transform, None)
    ...
    if args.small:
        config.tuneG['inv_steps'] = 10
        config.tuneG['tune_steps'] = 15
    ...
    for target_id in sorted(list(set(targets.tolist()))):
        ...
        for round_num in range(N):
            ...
            if config.logging:
                _ = write_list(
                    f'model_inversion/ppdg/results/{wandb_run_id}/inv_loss_r{round_num + 1}',
                    inv_dist_list)
            ...
            for i in tqdm(range(config.tuneG['tune_steps'])):
                ...
                kwargs = {
                    ...
                    'space_regulizer': space_regulizer
                }
                ...
                if i == 0 or (i + 1) % config.tuneG['tune_steps'] == 0:
                    ...
                    img.save(
                        os.path.join(f"model_inversion/ppdg/results/{wandb_run_id}/{target_id:03d}",
            ...
            if config.logging:
                _ = write_list(
                    f'model_inversion/ppdg/results/{wandb_run_id}/tune_loss_r{round_num + 1}',
                    inv_dist_list
                )
            ...
            if config.logging:
                _ = write_list(
                    f'model_inversion/ppdg/results/{wandb_run_id}/time_cost_r{round_num + 1}',
                    time_cost_list
                )
    ...
    ### log avg_dist_facenet across classes, as well as overall acc_top1, acc_top5, and avg_total_conf across all classes
    if config.logging:
        for round_num in range(N):
            # load accuracy summary csv and take means over targets
            acc_csv_path = f"model_inversion/ppdg/results/{wandb_run_id}/accuracy_summary_r{round_num + 1}.csv"
            acc_df = pd.read_csv(acc_csv_path)
            round_acc_top1_all_targets = acc_df['acc_top1'].mean()
            round_acc_top5_all_targets = acc_df['acc_top5'].mean()
            round_avg_total_conf_all_targets = acc_df['avg_total_conf'].mean()

            # load distance facenet list csv and take mean over targets
            distance_facenet_path = f"model_inversion/ppdg/results/{wandb_run_id}/distance_facenet_r{round_num + 1}.csv"
            distance_df = pd.read_csv(distance_facenet_path)
            round_avg_dist_facenet_all_targets = distance_df['mean_dist'].mean()

            if wandb_run:
                wandb.log({
                    f'r{round_num + 1}_evaluation_acc@1': round_acc_top1_all_targets,
                    f'r{round_num + 1}_evaluation_acc@5': round_acc_top5_all_targets,
                    f'r{round_num + 1}_avg_total_conf': round_avg_total_conf_all_targets,
                    f'r{round_num + 1}_avg_dist_facenet': round_avg_dist_facenet_all_targets,
                })

                if round_num == N - 1:
                    wandb.log({
                        'evaluation_acc@1': round_acc_top1_all_targets,
                        'evaluation_acc@5': round_acc_top5_all_targets,
                        'avg_total_conf': round_avg_total_conf_all_targets,
                        'avg_dist_facenet': round_avg_dist_facenet_all_targets,
                    })
    ...
    for i, (final_w, final_targets) in enumerate(zip(all_final_w, all_final_targets)):
        ...
        try:
            ...
            # training_dataset = create_target_dataset(target_dataset_name,
                                                    #  target_transform)
            training_dataset = target_dataset #! not sure if this will work
            ...
            #     _ = write_list(
            #         f'model_inversion/ppdg/results/{wandb_run_id}/prdc_list_r{i + 1}', prdc_list)
            # compute FID score
