...
class PRDC:
    def __init__(self, dataset_real, dataset_fake, device, batch_size=128, dims=2048, num_workers=16, gpu_devices=[]):
    ...
    def compute_metric(self, num_classes, k=3, rtpt=None):
        ...
        for step, cls in enumerate(range(num_classes)):
            with torch.no_grad():
                ...
                if rtpt:
                    rtpt.step(
                        subtitle=f'PRDC Computation step {step} of {num_classes}')
